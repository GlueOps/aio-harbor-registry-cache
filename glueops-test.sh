...
usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --env=ENV_NAME        Use predefined environment config (local, github, production, uswest2, useast1)"
    ...
    echo "  $0 --env=production                # Use production environment"
    echo "  $0 --env-file=custom.env           # Use custom environment file"
    echo ""
    echo "Available predefined environments:"
    echo "  local       - Local development (default)"
    echo "  github      - GitHub Actions CI"
    echo "  production  - Production deployment (requires config/production.env)"
    echo "  uswest2     - US West 2 region (requires config/uswest2.env)"
    echo "  useast1     - US East 1 region (requires config/useast1.env)"
    exit 1
}
